...
export default function Navbar() {
  ...
  const handleMouseLeave = useCallback(() => {
    closeTimerRef.current = setTimeout(() => {
      setActiveDropdown(null);
    }, 150); // small delay prevents flicker when moving from button to dropdown
  }, []);
          ...
          {PRIMARY_NAV.map((item) => {
            ...
            return (
              ...
              >
                ...
                {/* Mega dropdown — pt-2 bridges the gap between button and dropdown */}
                {hasChildren && dropdownOpen && (
                  <div
                    className="absolute top-full left-0 pt-2 w-72 mega-menu-active rounded-[4px]"
                    onMouseEnter={() => handleMouseEnter(item.label)}
                    onMouseLeave={handleMouseLeave}
                  >
                    <div className="bg-white shadow-[0_2px_8px_rgba(0,0,0,0.04)] border border-[#EAECEB] p-2 rounded-[4px]">
