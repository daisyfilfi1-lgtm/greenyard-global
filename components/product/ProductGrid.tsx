  ...
  if (products.length === 0) {
    return (
      <div className="text-center py-16">
        <div className="w-16 h-16 mx-auto mb-4 rounded-full bg-[#F0EDE8] flex items-center justify-center">
        ...
        <div className="mb-6 p-4 bg-white rounded-lg border border-[#E8E8E0] flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex gap-2">
              {compareSkus.map(sku => (
                <span key={sku} className="inline-flex items-center gap-1 px-2.5 py-1 bg-[#F0EDE8] text-xs font-medium text-text-primary rounded-full">
                  {sku}
                  <button onClick={() => toggleCompare(sku)} className="hover:text-[#D4AF37]" aria-label={`Remove ${sku}`}>
                    &times;
                  </button>
                </span>
              ))}
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="px-4 py-2 text-sm font-semibold bg-[#D4AF37] text-white rounded-md hover:bg-[#E8D58A] transition-colors"
            >
              Compare Selected
            </button>
          </div>
